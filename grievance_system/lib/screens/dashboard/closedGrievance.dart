...
class closedGrievance extends StatefulWidget {
  const closedGrievance({super.key});

  @override
  State<closedGrievance> createState() => _closedGrievanceState();
}

class _closedGrievanceState extends State<closedGrievance> {
    ...
    _timer = Timer.periodic(Duration(seconds: 5), (timer) async {
      ...
      if (response.statusCode == 200) {
        ...
        _grievancesController.add(
          grievances.where((grievance) => grievance['status'] == 'Closed').toList(),
        );
      } else {
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final grievances = role == 'admin'
                    ? allGrievances.where((grievance) => grievance['status'] == 'Closed').toList() // Show only pending grievances for admin
                  ...
                  return (assignedTo == userEmail || submittedBy == userEmail) &&
                      (grievance['status'] == 'Closed'); // Filter by pending status
                }).toList();
                print(grievances);
