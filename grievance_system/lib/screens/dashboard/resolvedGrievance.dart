...
class resolvedGrievance extends StatefulWidget {
  const resolvedGrievance({super.key});

  @override
  State<resolvedGrievance> createState() => _resolvedGrievanceState();
}

class _resolvedGrievanceState extends State<resolvedGrievance> with AutomaticKeepAliveClientMixin{
    ...
    _timer = Timer.periodic(Duration(seconds: 5), (timer) async {
      ...
      if (response.statusCode == 200) {
        ...
        _grievancesController.add(
          grievances.where((grievance) => grievance['status'] == 'Resolved').toList(),
        );
      } else {
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final grievances = role == 'admin'
                    ? allGrievances.where((grievance) => grievance['status'] == 'Resolved').toList() // Show only pending grievances for admin
                  ...
                  return (assignedTo == userEmail || submittedBy == userEmail) &&
                      (grievance['status'] == 'Resolved'); // Filter by pending status
                }).toList();
                print(grievances);
