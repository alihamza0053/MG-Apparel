...
class inProgressGrievance extends StatefulWidget {
  const inProgressGrievance({super.key});

  @override
  State<inProgressGrievance> createState() => _inProgressGrievanceState();
}

class _inProgressGrievanceState extends State<inProgressGrievance> with AutomaticKeepAliveClientMixin {
    ...
    _timer = Timer.periodic(Duration(seconds: 5), (timer) async {
      ...
      if (response.statusCode == 200) {
        ...
        _grievancesController.add(
          grievances.where((grievance) => grievance['status'] == 'In Progress').toList(),
        );
      } else {
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final grievances = role == 'admin'
                    ? allGrievances.where((grievance) => grievance['status'] == 'In Progress').toList() // Show only pending grievances for admin
                  ...
                  return (assignedTo == userEmail || submittedBy == userEmail) &&
                      (grievance['status'] == 'In Progress'); // Filter by pending status
                }).toList();
                print(grievances);
